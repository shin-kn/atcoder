...
template <size_t P>
class FormalPowerSeries<Mod<P>>{
	public:
		Array<Mod<P>> arr;
		size_t Degree;
		FormalPowerSeries():Degree(0){};
		FormalPowerSeries(size_t N):Degree(N){
			arr.Allocate(N+1);
		};
		...
		FormalPowerSeries operator * (FormalPowerSeries& other){//convolution of Degree(deg)
			...
			if(P==998244353&&length<=(size_t)1<<23){
				...
				Mod<P> zeta=Power(Mod<P>().Set((size_t)15311432),((size_t)1<<(23-loglength)));
				
				LightArray<Mod<P>> arr1=fouriertransform_freq(arr,loglength,Mod<P>().Set(1)/zeta);
				
				LightArray<Mod<P>> arr2=fouriertransform_freq(other.arr,loglength,Mod<P>().Set(1)/zeta);
		...
		FormalPowerSeries Copy(){
			...
			for(size_t i=0;i<=Degree;++i) res[i]=arr[i];
			return res;
		}
...
template <size_t P>
using FPS=FormalPowerSeries<Mod<P>>;
...
Mod<P> BostanMori(FPS<P>& p,FPS<P>& q,size_t N){//[x^N] P(x)/Q(x), 
	...
	FPS<P> lower(degree);
	lower.Init();
	for(size_t i=0;i<=N&&i<=p.Degree&&i<=degree;++i) upper[i]=p[i];
	for(size_t i=0;i<=N&&i<=q.Degree&&i<=degree;++i) lower[i]=q[i];
	while(N!=0){
