...
template <typename T>
void Sort(Array<T>& arr,bool Smaller=true){
	if(Smaller){
...
void SortByKey(Array<T>& arr1,Array<U>& arr2,Array<V>& arr3,bool Smaller=true){
	class Wrapper{
		Array<T>* arr;
		public:
			size_t val;
			...
			inline bool operator < (Wrapper other){return (*arr)(val)<(*arr)(other.val);}
			inline bool operator > (Wrapper other){return (*arr)(val)>(*arr)(other.val);}
			inline bool operator == (Wrapper other){return (*arr)(val)==(*arr)(other.val);}
	};
	if(Smaller){
