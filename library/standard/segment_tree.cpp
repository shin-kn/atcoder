...
template <typename T> class SegmentTree {
	...
	public:
		...
		SegmentTree(ull N,AddFunc&& func){Init(N,std::forward<AddFunc>(func));}
		template <typename AddFunc>
		SegmentTree(ull N,T val,AddFunc&& func){Init(N,val,std::forward<AddFunc>(func));}
		template <typename AddFunc>
		SegmentTree(Array<T>& arr,AddFunc&& func){Init(arr,std::forward<AddFunc>(func));}


		template <typename AddFunc>
		void Init(ull N,AddFunc&& func){Init(N,T(),std::forward<AddFunc>(func));}
		template <typename AddFunc>
		void Init(ull N,T val,AddFunc&& func){Array<T> arr=Array<T>();Init(N,arr,val,std::forward<AddFunc>(func));}
		template <typename AddFunc>
		void Init(Array<T>& arr,AddFunc&& func){Init(arr.Length,arr,T(),std::forward<AddFunc>(func));}
		template <typename AddFunc>
...
template <typename T> ull SegmentTree<T>::CellIndex(size_t start,size_t logsize){
	return ((1<<(Degree-logsize-1))-1)+(start>>logsize);
}

template <typename T> Array<ull> SegmentTree<T>::GetRange(size_t start,size_t end){
	Array<ull> res;
	size_t loc=start;
	for(size_t i=0;true;++i){
		if((1<<i)+loc>end+1) break;
		if((1<<i)&loc){
...
class LazySegmentTree {
	...
	public:
		...
		LazySegmentTree(ull N,T val,AddFunc&& addfunc,Func&& func,ConvoluteFunc&& convolutefunc){Init(N,val,std::forward<AddFunc>(addfunc),std::forward<Func>(func),std::forward<ConvoluteFunc>(convolutefunc));}

		template <typename AddFunc,typename Func, typename ConvoluteFunc>
		void Init(ull N,T val,AddFunc&& addfunc,Func&& func,ConvoluteFunc&& convolutefunc);
...
template <typename T,typename U>
template <typename AddFunc,typename Func, typename ConvoluteFunc>
void LazySegmentTree<T,U>::Init(ull N,T init_val,AddFunc&& addfunc,Func&& funcfunc,ConvoluteFunc&& convolutefunc){
	add=FunctionType<T(T,T)>(std::forward<AddFunc>(addfunc));
	func=FunctionType<T(U,U)>(std::forward<Func>(funcfunc));
...
template <typename T,typename U> inline void LazySegmentTree<T,U>::GetRange(ull start,ull end){
	ToBeExecuted.Clear();
	ToBeRefreshed.Clear();
	TargetRange.Clear();	
...
template <typename T,typename U> inline void LazySegmentTree<T,U>::Propagate(ull loc){
	if(ActIsNull[loc]) return;
	ActIsNull[loc]=true;
	if(Child(loc)<CellLength){
		if(!ActIsNull[Child(loc)]) Act[Child(loc)]=convolute(Act[loc],Act[Child(loc)]);
		else{Act[Child(loc)]=Act[loc];ActIsNull[Child(loc)]=false;}
		if(!ActIsNull[Child(loc)+1]) Act[Child(loc)+1]=convolute(Act[loc],Act[Child(loc)+1]);
		else{Act[Child(loc)+1]=Act[loc];ActIsNull[Child(loc)+1]=false;}
	}
}


template <typename T,typename U> inline void LazySegmentTree<T,U>::Execute(ull loc){
	if(ActIsNull[loc]) return;
	Val[loc]=func(Act[loc],(Val[loc]));
	Propagate(loc);
}

template <typename T,typename U> inline void LazySegmentTree<T,U>::Refresh(ull loc){
	if(Child(loc)>=CellLength) return;
	Val[loc]=add(Val[Child(loc)],Val[Child(loc)+1]);
}

template <typename T,typename U> inline void LazySegmentTree<T,U>::ExecuteAndRefreshAll(){
...
template <typename T,typename U> inline T LazySegmentTree<T,U>::Eval(ull start,ull end){
	GetRange(start,end);
...
template <typename T,typename U> inline void LazySegmentTree<T,U>::Action(U action,ull start,ull end){
	GetRange(start,end);
	while(TargetRange.Size>0){
...
template <typename T,typename U> inline void LazySegmentTree<T,U>::Set(ull loc,T val){
