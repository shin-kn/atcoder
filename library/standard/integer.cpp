...
inline T Sqrt(T num){//assume num>=0
...
template<typename T>
inline T Combination(T n,T c){
	if(c>=n/2+1) return Combination(n,n-c);
	T counter=1;
...
template <typename T>
inline Set<T,2> AxBy(T a,T b) {//a!=0,b!=0
	if(a==0||b==0) return Set<T,2>(0,0);
	if(Abs(a)<Abs(b)){
		T rem=b%a;
		if(rem==0){
	...
	public:
		...
		Array<Set<size_t,2>> operator ()(size_t num){
			...
			for(size_t i=0;num!=1;++i){
				...
				if(counter!=0) arr.Push(Set<size_t,2>(j,counter));
			}
			return arr;
		}
