...
class Tree {
    public:
        ...
        void DFS(ull start,FunctionType<void(ull)> backfunc,FunctionType<bool(ull,ull)> cango,FunctionType<void(ull)> firstreach);


		template <typename func1,typename func2,typename func3>
		void DFS(ull start,func1&& backfunc, func2&& cango,func3&& firstreach){DFS(start,FunctionType<void(ull)>(std::forward<func1>(backfunc)),FunctionType<bool(ull,ull)>(std::forward<func2>(cango)),FunctionType<void(ull)>(std::forward<func3>(firstreach)));}
...
template <typename T>
void Tree<T>::Connect(ull parent,ull child,T val){
...
template <typename T>
void Tree<T>::DFS(ull start,FunctionType<void(ull)> backfunc,FunctionType<bool(ull,ull)> cango,FunctionType<void(ull)> firstreach){
    Array<ull,true> loc;
    Array<ull,true> num;
    ull place=0;
    loc(0)=start;
...
template <typename T>
ull Tree<T>::LCA(ull node1,ull node2){
    if(!canLCA) Memo();
    if(Nodes[node1].depth<Nodes[node2].depth) Swap(node1,node2);
    node1=JumpParent(node1,Nodes[node1].depth-Nodes[node2].depth);
    if(node1==Root) return node1;
    ull depth=Nodes[node1].depth;
    ull degmax=Log2(SmallerPower2(depth));
    if(node1==node2) return node1;
    ull succeed=Root;
    for(ull deg=degmax;;--deg){
        if(Nodes[node1].depth<(1ull<<deg)) continue;
...
template <typename T>
T Tree<T>::Distance(ull node1,ull node2){
    if(!canLCA) Memo();
    ull ancestor=LCA(node1,node2);
    return JumpToParent(node1,Nodes[node1].depth-Nodes[ancestor].depth)+JumpToParent(node2,Nodes[node2].depth-Nodes[ancestor].depth);
}
...
void Tree<T>::Memo(){
    ...
    for(ull degree=1;;++degree){
        ...
        for(ull i=0;i<stack.Length;++i){
            ...
            locnode.parents.Push(Nodes[locnode.parents[degree-1]].parents[degree-1]);
            locnode.toparents.Push(locnode.toparents[degree-1]+Nodes[locnode.parents[degree-1]].toparents[degree-1]);
        }
        if(newstack.Length==0) break;
...
template <typename T>
ull Tree<T>::JumpParent(ull node,ull jump){
...
template <typename T>
T Tree<T>::JumpToParent(ull node,ull jump){
    if(jump==0) return T();
    ull degmax=Log2(SmallerPower2(jump));
    T counter=T();
    for(ull deg=0;deg<=degmax;++deg){
