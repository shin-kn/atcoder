...
template <typename T,bool Unsafe=false> class Array {
	...
	public:
        ...
        Array(){}
		Array(size_t n){Allocate(n);Length=n;}
		Array(size_t n,T val){Allocate(n);Length=n;Set(val);}
...
template <typename T, bool Unsafe> Array<T,Unsafe>::~Array(){
	...
	if(pointers!=nullptr) delete [] pointers;


}

template <typename T, bool Unsafe> Array<T,Unsafe>::Array(LightArray<T>&& other){
	Length=other.Length;
	Capacity=other.Length;
	pointerscapacity=1;
	pointerslength=1;	
	pointers=new (std::nothrow) T*[pointerscapacity];
	if(pointers==nullptr) std::exit(EXIT_FAILURE);
...
template <typename T, bool Unsafe> void Array<T,Unsafe>::Set(ull start,ull end, T val){
...
template <typename T, bool Unsafe> inline T& Array<T,Unsafe>::operator [] (ull idx){
	if constexpr (!Unsafe){
		if(Length<=idx){
			if(Capacity<=idx){
				Allocate(idx+1);
...
template <typename T, bool Unsafe> inline void Array<T,Unsafe>::Allocate(ull cap){
	...
	if(pointerslength==pointerscapacity){
		...
		if(oldpointers!=nullptr) delete [] oldpointers;
	}
	T* newpointer=new (std::nothrow) T[cap-Capacity];
	if(newpointer==nullptr) std::exit(EXIT_FAILURE);
...
template <typename T, bool Unsafe> Array<T,Unsafe>::Array(Array<T,Unsafe>&& src){
	arr = src.arr;
	Length=src.Length;
...
template <typename T, bool Unsafe> Array<T,Unsafe>& Array<T,Unsafe>::operator = (Array<T,Unsafe> &&src){
	if (&src == this) {
		return *this;
	}
	//destructor
